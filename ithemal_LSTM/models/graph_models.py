...
class AbstractGraphModule(nn.Module):
    ...
    def init_hidden(self):
        ...
        return (
            nn.Parameter(torch.zeros(1, 1, self.hidden_size, requires_grad=True)).cuda(),
            nn.Parameter(torch.zeros(1, 1, self.hidden_size, requires_grad=True)).cuda(),
        )
    ...
    def init_bblstm(self, item):
        ...
        for i, instr in enumerate(item.block.instrs):
            ...
            if self.mode == 'learnt':
                ...
                instr.tokens = self.final_embeddings(torch.cuda.LongTensor(tokens))
...
class RNN(AbstractGraphModule):
    ...
    def forward(self, item):
        ...
        for instr, token_inputs in zip(item.block.instrs, item.x):
            ...
            tokens = self.final_embeddings(torch.cuda.LongTensor(token_inputs)).unsqueeze(1)
